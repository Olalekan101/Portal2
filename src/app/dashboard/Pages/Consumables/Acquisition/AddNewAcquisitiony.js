...
function Acquisition() {
  ...
  const defaultData = {
    ...
    AcquisitionDate: location?.acquisitionDate,
    DepreciationValue: location?.depreciationValue,
    DepreciationStartDate: location?.depreciationStartDate,
    DepreciationEndDate: location?.depreciationEndDate,
    State: location?.state,
    NVB: location?.nvb,
    Status: location?.assetStatus,
    ...
    InsuranceType: location?.insuranceType,
    InsurancAddress: location?.insuranceAddress,
    InsuranceEmail: location?.insuranceEmail,
  ...
  const submit = (data) => {
    ...
    const combinedData = {
      ...data,
      VendorName:
        assetItems?.name || assetItems?.businessName || assetItems?.companyName,
    };
            ...
            <div className={DashboardStyle.inputs_group}>
              ...
              <div>
                <FormSelect
                  ...
                  placeholder={"select"}
                  isOptional={true}
                  array={assetTypes?.responseObject?.map?.((x, index) => (
                    <option key={index} value={x?.id}>
                      {x?.name}
                    </option>
                  ))}
