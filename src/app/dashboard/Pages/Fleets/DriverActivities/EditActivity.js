...
function AddActivity({ isOpen, setIsOpen, apiData }) {
  ...
  const { all_fleets, isLoadingActivity } = useSelector((state) => state?.fleet);
  ...
  const submit = (data) => {
    ...
    //return
    dispatch(UpdateDriverActivity(fileData))?.then((res) => {
  ...
  return (
    <PageLayout title={"Update Driver Activity"} hasBack={true}>
      <FormProvider {...formMethods}>
        <FormTemplate handleSubmit={handleSubmit(submit)}>
            ...
            <div className={DashboardStyle.inputs_group}>
              <div>
                <FormsSelect
                  ...
                  moreRegister={{
                    onChange: (e) => {
                      const driver =  filteredNames?.find(
          ...
          <div className={DashboardStyle.button_cage}>
            <SupportButtons width={"auto"}>Save as Draft</SupportButtons>
            <ActionButtons
              disabled={!isValid}
              isLoading={isLoadingActivity}
              className={DashboardStyle?.button_cage_weight}
            >
              {isLoadingActivity ? "Submitting" : "Submit"}
            </ActionButtons>
          </div>
        </FormTemplate>
      </FormProvider>
