...
export function QuotationModals({
  ...
  switch (type) {
    case "approve":
      modalContent = <ApproveProcQuotations closeModal={closeModal} />;
      break;

    case "decline":
      modalContent = <DeclineProcQuotations closeModal={closeModal} />;
      break;
...
function ApproveProcQuotations({
  declineAction,
  approvalAction,
  isOpen,
  closeModal,
  ...
  return (
    <>
      <div>
        ...
        <h3 className={Modal?.modal_action_h3}>Approve Quotation?</h3>
...
function DeclineProcQuotations({ closeModal }) {
  const formMethods = useForm({
    mode: "all",
  });
  const {
  ...
  return (
    <>
      <div>
        <img className={Modal?.modal_action_img} src={Action} alt="action" />
        <h3 className={Modal?.modal_action_h3}>Decline Quotation?</h3>
        <hr className={Modal?.modal_action_hr} />
        <FormProvider {...formMethods}>
          <FormTemplate handleSubmit={handleSubmit(submit)}>
            <FormTextArea
